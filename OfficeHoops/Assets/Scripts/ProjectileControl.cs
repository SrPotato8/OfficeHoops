...
public class ProjectileControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) && cameraScript.movingUp)
        {
            chargingForce = true;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Hoop") && !fireworkPlayed)
        {
            ...
            //Close cover after scoring
            HoopCover hoopCover = collision.gameObject.GetComponent<HoopCover>();
            if (hoopCover != null && hoopCover.binCover != null)
            {
                CloseCover(hoopCover.binCover);
            }
    ...
    void CloseCover(GameObject cover)
    {
        HingeJoint hinge = cover.GetComponent<HingeJoint>();
        if (hinge != null)
        {
            ...
            Debug.Log("Cover closing");
        }
        // Wait a short moment before freezing it
        StartCoroutine(DisableHingeSpringAfterDelay(hinge, 0.5f));
    }

    IEnumerator DisableHingeSpringAfterDelay(HingeJoint hinge, float delay)
    {
